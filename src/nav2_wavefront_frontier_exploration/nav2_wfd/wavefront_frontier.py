...
def main(argv=sys.argv[1:]):
    ...
    while not test.initial_pose_received:
        test.info_msg('Waiting for odometry pose')
        rclpy.spin_once(test, timeout_sec=1.0)

    while test.costmap is None:
        test.info_msg('Waiting for map')
        rclpy.spin_once(test, timeout_sec=1.0)

    while not test.action_client.wait_for_server(timeout_sec=1.0):
        test.info_msg("Waiting for 'follow_waypoints' action server")
